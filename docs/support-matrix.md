...
| `briefloop run --workspace <path> --runtime codex` plus `runtime next/continue/invocation-start/invocation-validate/invocation-accept/invocation-fail/apply/diagnose`; strict JSON proposal contract preflight | Experimental; fresh SQLite-only, no JSON migration or fallback. Store-authorized `runtime continue` supports imported local manifests and init-web Tavily discovery; Tavily raw page content may enter one atomic source-promotion receipt, while search snippets remain ineligible discovery material |
