...
| `briefloop run --workspace <path> --runtime codex` plus `runtime next/continue/invocation-start/invocation-validate/invocation-accept/invocation-fail/apply/diagnose`; strict JSON proposal contract preflight | Experimental; fresh SQLite-only, no JSON migration or fallback. Store-authorized `runtime continue` supports imported local manifests and init-web Tavily discovery on retained-publication-capable POSIX/macOS hosts; Tavily raw page content may enter one atomic source-promotion receipt, while search snippets remain ineligible discovery material. On Windows, local initialization, discovery authorization, and the workspace `.env` credential may already be committed; the packaged continuation then stops before any Tavily call or other provider/network call with retry-stable `checkout_publication_unsupported`, zero provider calls, and no source promotion, execution authorization, finalization, approval, package, delivery, or external publication |
