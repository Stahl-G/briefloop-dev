...
def test_migration_resource_matches_packaged_source_text() -> None:
    ...
    migration_8 = source.with_name("0008.sql")
    packaged_8 = resources.files("multi_agent_brief.control_store").joinpath(
        "migrations", "0008.sql"
    )
    assert packaged_8.read_text(encoding="utf-8") == migration_8.read_text(
        encoding="utf-8"
    )
    migration_9 = source.with_name("0009.sql")
    packaged_9 = resources.files("multi_agent_brief.control_store").joinpath(
        "migrations", "0009.sql"
    )
    assert packaged_9.read_text(encoding="utf-8") == migration_9.read_text(
        encoding="utf-8"
    )
