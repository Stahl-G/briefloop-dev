...
def test_init_web_manual_handoff_retains_standard_codex_run(
    monkeypatch, capsys, tmp_path: Path
) -> None:
    selected = tmp_path / "manual"
    server = _InitWebServerDouble(
        outcome=SimpleNamespace(
            status="committed",
            workspace=str(selected),
...
def test_init_web_both_authorities_handoff_uses_runtime_continue(
    monkeypatch, capsys, tmp_path: Path
) -> None:
    selected = tmp_path / "promoted"
    server = _InitWebServerDouble(
        outcome=SimpleNamespace(
            status="replayed",
            workspace=str(selected),
            run_id="RUN-PROMOTED",
            transaction_id="TX-PROMOTED",
            execution_authorized=True,
            source_discovery_authorized=True,
        )
    )
